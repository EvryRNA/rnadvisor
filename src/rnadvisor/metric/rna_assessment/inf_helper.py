...
class INFHelper(AbstractAssessment):
    ...
    @staticmethod
    def compute_inf_all_from_structures(
        ...
        inf_all = comparer.INF(
            src_struct=pred_struc, trg_struct=native_struc, type="ALL"
        )
        if inf_all == -1:
            inf_all = np.nan
        return inf_all
    ...
    @staticmethod
    def compute_inf_wc_from_structures(
        native_struc: PDBStruct, pred_struc: PDBStruct
    ) -> float:
        """
        ...
        comparer = PDBComparer()
        inf_wc = comparer.INF(
            src_struct=pred_struc, trg_struct=native_struc, type="PAIR_2D"
        )
        if inf_wc == -1:
            inf_wc = np.nan
        return inf_wc
    ...
    @staticmethod
    def compute_inf_nwc_from_structures(
        native_struc: PDBStruct, pred_struc: PDBStruct
    ) -> float:
        """
        ...
        """
        comparer = PDBComparer()
        inf_nwc = comparer.INF(
            src_struct=pred_struc, trg_struct=native_struc, type="PAIR_3D"
        )
        if inf_nwc == -1:
            inf_nwc = np.nan
        return inf_nwc
    ...
    @staticmethod
    def compute_inf_stack_from_structures(
        native_struc: PDBStruct, pred_struc: PDBStruct
    ) -> float:
        """
        Compute the INF score only for the non WC base-pairing
        ...
        """
        comparer = PDBComparer()
        inf_stack = comparer.INF(
            src_struct=pred_struc, trg_struct=native_struc, type="STACK"
        )
        if inf_stack == -1:
            inf_stack = np.nan
        return inf_stack
    ...
    def compute_inf_stack(
        ...
        inf_stack = INFHelper.compute_inf_stack_from_structures(
