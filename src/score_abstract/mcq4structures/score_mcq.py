...
class ScoreMCQ(ScoreAbstract):
    ...
    @staticmethod
    def compute_mcq(pred_path: str, native_path: str, mcq_bin_path: Optional[str] = None) -> float:
        """
        ...
        command = (
            f"{mcq_bin_path} -t {native_path} -d tmp {pred_path}"
    ...
    def _compute(self, pred_path: str, native_path: str) -> Tuple[Dict, Dict]:
        ...
        """
        mcq_score = self.compute_mcq(pred_path, native_path, self.mcq_bin_path)
