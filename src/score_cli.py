...
from src.enum import CONVERT_NAME_TO_SCORING_CLASS
...
class ScoreCLI:
    ...
    def convert_cli_args_scores(all_scores: str, *args, **kwargs) -> Dict:
        ...
        if all_scores == "ALL":
            # Get all the names
            all_scores_split = list(CONVERT_NAME_TO_SCORING_CLASS.keys())
        else:
            all_scores_split = all_scores.upper().split(",")
    ...
    def convert_cli_args(config_path: Optional[str], *args, **kwargs) -> Dict:
        ...
        pred_path, native_path, result_path, time_path = (
        ...
        all_scores = ScoreCLI.convert_name_method_to_class(all_scores, **bin_paths)
        config = {
            "pred_path": pred_path,
    ...
    @staticmethod
    def convert_name_method_to_class(
        all_scores: Optional[List], *args, **kwargs
    ) -> Optional[List[ScoreAbstract]]:
        """
        Convert a list of name of methods to an instantiation of class
        :param all_scores: a list of names of scoring method
        :return: a list of ScoreAbstract instances
        """
        if all_scores is None or all_scores == "ALL":
            logger.debug("NO SCORES SPECIFIED: USE ALL SCORES")
            all_scores = list(CONVERT_NAME_TO_SCORING_CLASS.keys())
        new_scores = []
        for score_name in all_scores:
            new_score = CONVERT_NAME_TO_SCORING_CLASS.get(score_name, None)(*args, **kwargs)
    ...
    def compute_scores(self, mean_max_min: bool = False):
        ...
        self._save_scores(score_df, self.result_path)
        self._save_scores(times_df, self.time_path)
    ...
    def _save_scores(self, score_df: pd.DataFrame, result_path: Optional[str] = None):
        """
        Save the scores in a dataframe in the result_path
        :param score_df: the different scores to save
        """
        if result_path is None:
            logger.debug("NO LOG PATH, NO SAVING")
            return None
        score_df.to_csv(result_path)
        logger.debug(f"SAVED AT {result_path}")
